...
projects.map((item, index) => {
  projectDiv.innerHTML += `<div key=${index} class="relative s-shadow  rounded-md lg:rounded-xl fade-up">
          <img class="object-cover rounded-md lg:rounded-xl" src=${
            item.image
          } />
          <div class="foreground-overlay">
            <h2 class="text-primary text-base md:text-2xl my-1 md:my-2 font-bold">${
              item.name
            }</h2>
  ...
  (item, index) =>
    (certificateDiv.innerHTML += ` <div key=${index} class="h-32 md:h-52  lg:h-64 relative fade-up rounded-md lg:rounded-xl certificate-container s-shadow">
          ...
          <div class="foreground-overlay">
            ...
            <p class="text-sm text-center lg:text-lg text-txt-color">${
              item.name
            }</p>
          </div>
        </div>`)
...
function sendEmail() {
  ...
  emailjs.send("service_ax416gh", "template_7vw1tig", templateParams).then(
    function () {
    ...
    function () {
      ...
      error.innerText = "Something went wrong! Please try after sometime.";
      setTimeout(function () {
...
popupTriggers.forEach((trigger) => {
  trigger.addEventListener("click", function (event) {
    ...
    const imgSrc = this.closest(".certificate-container").querySelector(
      "img"
    ).src;
